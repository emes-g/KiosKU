...
public class KioskView extends JFrame {
	...
	public int[] inputCurrency() {
		 JTextField[] coinFields = new JTextField[8];
	...
	public void orderWindow(OrderManager om, Order o) {
		...
		for(int i = 0; i < itemList.size(); i++) {
			...
			//JPanel itemPanel = createPanel(item.getTitle());
	...
	public JPanel createItemPanel(JPanel basketPanel, JPanel totalPricePanel, Order o, Item item) {
		//lblNewLabel.setBounds(10, 14, 50, 15);
        ...
        while (!validInput) {
            ...
            try {
                ...
                if (numPeople <= 6) {
                    ...
                    JOptionPane.showMessageDialog(null, "최대 6명까지 가능합니다.", "오류", JOptionPane.ERROR_MESSAGE);
                }
            } catch (NumberFormatException e) {
                // 숫자로 변환할 수 없는 경우
	...
	public void displayReceipt(Receipt receipt, int num) {
        ...
        // count 배열에 저장된 동전 개수를 JLabel로 추가
        //panel.add(new JLabel(receipt.getReceiptInfo()));
